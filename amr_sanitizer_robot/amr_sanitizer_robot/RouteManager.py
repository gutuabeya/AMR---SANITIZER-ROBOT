...
class RouteManager(Node):
   ...
   Format:
    ...
    def __init__(self):
        ...
        route_file_path = os.path.join(os.getcwd(), 'src', 'amr_sanitizer_robot', 'amr_sanitizer_robot', 'routes.yaml')
    ...
    def get_result_callback(self, future: NavigateToPose.Result):
        ...
        if self.number_of_goals < self.length:
            ...
            self.get_logger().info("No more goals, stop route manager manually from Terminal.")
...
def main():
    ...
    try:
        ...
        while route_manager.localization_complete == False:
            if count==0:
                route_manager.get_logger().info("Waiting untill localization to complete")
                count=1
            rclpy.spin_once(route_manager)
        route_manager.get_logger().info("Localization complete, Lets start RouteManager")
